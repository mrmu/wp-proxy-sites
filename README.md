...
1. 503 Service Temporarily Unavailable：通常是網址請求對應不到 Server 裡的網址設定才會發生。請檢查 DNS 設定的網址與 yml 裡的 VIRTUAL_HOST 設定是否完全一致；另外有可能是 Docker image 有問題，可以刪掉舊的 Docker image (建議先不刪 DB 的 image，有時版本不同可能造成之後資料庫檔案不相容)，讓它重抓。
